...
class Fast_SCNN:
    def __init__(self, num_classes=4, input_shape=(256, 256, 3)):
    ...
    def pyramid_pooling_block(self, input_tensor, bin_sizes):
        ...
        for bin_size in bin_sizes:
            ...
            x = keras.layers.BatchNormalization()(x)
            x = keras.layers.ReLU()(x)
            # x = keras.layers.Lambda(lambda image: ktf.image.resize_images(image, (h, w)))(x)
    ...
    def classifier(self):
        ...
        # self.classifier = keras.layers.UpSampling2D((8, 8))(self.classifier)
        self.classifier = keras.layers.Lambda(lambda image: ktf.image.resize_images(image, (256, 256)), name='Resize')(self.classifier)
    ...
    def model(self, show_summary=True, activation='softmax'):
        ...
        fast_scnn = keras.Model(inputs=self.input_layer,
